defmodule AttackTest do
  ...
  test "it crits when roll is natural 20", context do
    {result, attacker, defender} = Attack.attack(context[:attacker], context[:defender], 20)
    assert result == :critical
  end

  test "it does not damage oppoent on a miss", context do
    {:miss, attacker, defender} = Attack.attack(context[:attacker], context[:defender], 5)
    assert Hero.hit_points(defender) == 5
  end

  test "it damages opponent on a hit", context do
    {:hit, attacker, defender} = Attack.attack(context[:attacker], context[:defender], 15)
    assert Hero.hit_points(defender) == 4
  end

  test "it doubly damages opponent on a critical", context do
    {:critical, attacker, defender} = Attack.attack(context[:attacker], context[:defender], 20)
